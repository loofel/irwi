...
module Irwi::Support::RouteMapper
  ...
  def wiki_root( root, config = {} )
    ...
    get( "#{root}/format", opts.merge({ :action => 'format', :as => 'format_wiki_page' }) ) # Wiki history route
    ...
    get( "#{root}/delete/(*path)", opts.merge({ :action => 'destroy', :as => 'destroy_wiki_page' }) ) # Wiki destroy route
